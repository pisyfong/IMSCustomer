...
class PackBalanceService {
  /// Returns {key → totalAllocatedQty} for the company. `qty` (the planned
  /// pick amount) is summed, not `packQty` — a fully-allocated SQ line has no
  ...
  static Future<Map<String, double>> allocatedByLine({
    ...
    // A CANCELLED pack releases its claim, so skip its lines too.
    ...
    for (final it in rows) {
      ...
      if (cancelled.contains(it.packPreLabel)) continue; // cancelled pack
      if (excludePickPreLabel != null &&
          it.packPreLabel == excludePickPreLabel) {
        continue;
      }
