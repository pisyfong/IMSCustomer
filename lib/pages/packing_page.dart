...
class _PackingPageState extends State<PackingPage>
  ...
  Future<void> _loadCustomers(List<PackList> docs) async {
    final cc = _companyCode;
    if (cc == null || docs.isEmpty) return;
    final labels =
        docs.map((e) => e.packPreLabel).whereType<String>().toSet().toList();
    ...
    final items = await isar.packListItems
        ...
        .and()
        .anyOf(labels, (q, l) => q.packPreLabelEqualTo(l))
        .findAll();
    final sqOf = <String, Set<String>>{};
    for (final it in items) {
      final doc = it.packPreLabel;
  ...
  List<PackList> _applySort(List<PackList> rows) {
    final out = [...rows];
    out.sort((a, b) {
      final da = _sortKey(a);
      final db = _sortKey(b);
                ...
                title: Text(o.$1,
                    style: TextStyle(
                        ...
                        fontWeight: _sortBy == o.$2
                            ? FontWeight.w800
                            : FontWeight.w600,
                        color: _sortBy == o.$2 ? AppDesign.modPacking : AppDesign.ink)),
                trailing: _sortBy == o.$2
                    ? Icon(Icons.check, size: 18, color: AppDesign.modPacking)
                    : null,
              ),
  ...
  List<PackList> _filter(List<PackList> rows) {
    var out = rows;
    ...
    if (_query.isNotEmpty) {
      out = out.where((p) {
        final l = (p.packPreLabel ?? '').toLowerCase();
        final loc = (p.locationCode ?? '').toLowerCase();
        // Also match the customer(s) behind the document.
        final cust = (_docCustomers[p.packPreLabel ?? ''] ?? const <String>[])
            .join(' ')
            .toLowerCase();
  ...
  ///   1. Existing pack list  → open its detail page.
  ///   2. Pick list           → open the pack flow for the pick's source SQ
  ///                            (the pick shows up as a source on its lines).
  ///   3. Sales quote         → create a draft pack and jump into the
  ///                            "From SQ" flow pre-navigated to that SQ.
  ///   4. Sales invoice       → create a draft pack pre-loaded with whatever
  ///                            the invoice still has left to pack.
  ///   5. Nothing             → drop into the search field as a filter.
  ///
  /// The invoice is tried LAST because a document number that matches both a
  /// quote and an invoice should keep resolving the way it always has.
  Future<void> _dispatchCode(String code) async {
    ...
    final packMatch = [..._open, ..._completed].firstWhere(
      (p) => (p.packPreLabel ?? '').toLowerCase() == asLower,
      orElse: PackList.new,
    );
    if (packMatch.packPreLabel != null) {
      _searchCtrl.clear();
      await Navigator.push(
        context,
        MaterialPageRoute(builder: (_) => PackDetailPage(pick: packMatch)),
      );
      await _loadLocal();
      return;
    }
    ...
    if (_companyCode != null) {
      // A scanned PICK → open a draft pack pre-loaded with ALL its items
      // (across every source SQ), each attached to this pick.
      final pick = await isar.pickLists
          .filter()
          .companyCodeEqualTo(_companyCode)
          .and()
          .pickPreLabelEqualTo(trimmed)
          .findFirst();
      if (pick != null) {
        _searchCtrl.clear();
        await _openDraftForPick(pick);
        return;
      }
      ...
      // Invoice.companyCode is non-nullable, unlike the pick/quote models, and
      // a field can't type-promote through the enclosing null check.
      final cc = _companyCode!;
      final si = await isar.invoices
          .filter()
          .companyCodeEqualTo(cc)
          .and()
          .invoicePreLabelEqualTo(trimmed)
          .findFirst();
      if (si != null && mounted) {
        _searchCtrl.clear();
        await _openDraftForSi(si);
        return;
      }
    }
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
          content: Text(
              'No pack, pick, SQ or SI matches "$trimmed" — filtered')),
    );
  }

  /// Scanned a SALES INVOICE → new draft pack pre-loaded with everything the
  /// invoice still has left to pack.
  ///
  /// These lines have no parent pick. An invoice raised in the back office was
  /// never picked through this app, which is the whole reason packing from one
  /// exists — see [FromSiTab].
  ///
  /// Quantities are net of what other packs already claim, so scanning the same
  /// invoice twice offers the remainder rather than duplicating the work.
  Future<void> _openDraftForSi(Invoice si) async {
    final cc = _companyCode;
    if (cc == null) return;
    final label = si.invoicePreLabel;

    // A voided invoice has nothing to ship. Settlement status (A open,
    // F settled, P part-settled) does NOT gate packing — that describes
    // payment, not whether the goods have left.
    if (isInvoiceVoided(si.status)) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Invoice $label was cancelled')),
        );
      }
      return;
    }
    ...
    final lines = (await isar.invoiceItems
            ...
            .companyCodeEqualTo(cc)
            .and()
            .invoicePreLabelEqualTo(label)
            .findAll())
        // Invoice lines mark a cancellation 'C', not the 'X' used by picks
        // and packs.
    ...
    if (lines.isEmpty) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Invoice $label has no items to pack')),
        );
      }
      return;
    }
    ...
    final items = <PackListItem>[];
    for (final it in lines) {
      // FOC ships, so it is packed.
      final invoiced = (it.quantity ?? 0) + (it.foc ?? 0);
      final taken =
          allocated['$label|${SiPackBalanceService.keyFor(it.skuNo, it.uom)}'] ??
              0;
      final remaining = invoiced - taken;
      if (remaining <= 0.0001) continue;
      items.add(PackListItem()
        ..companyCode = cc
        ..skuNo = it.skuNo
        ..uom = it.uom
        ..parentPreLabel = null // no pick behind this invoice
        ..sourceSq = null
        ..sourceSi = label
        ..locationCode = (it.locationCode ?? si.locationCode)?.trim() ??
            _locationCode
        ..status = 'A'
        ..factor = it.factor
        ..qty = remaining
    ...
    if (items.isEmpty) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Invoice $label is already fully packed')),
        );
      }
      return;
    }

    // No pack number yet — it's minted on Save, so abandoning the draft
    // doesn't burn a running number.
    final draft = PackList()
      ..companyCode = cc
      ..status = 'A'
      ..addedDate = now
      ..totalPackQty = 0
      ...
      ..lastModifiedBy = _userId
      ..lastWriteTimeStamp = now
      ..locationCode = (si.locationCode ?? '').trim().isNotEmpty
          ? si.locationCode!.trim()
          : _locationCode
      ..assignedTo = _userId
      ..isSynced = false;
    if (!mounted) return;
  ...
  /// Scanned a pick → new draft pack pre-loaded with every one of the pick's
  /// (non-cancelled) lines, kept SEPARATE (each tagged with its Source_SQ) so
  /// the same SKU from two SQs stays as two lines. Parent = the pick.
  Future<void> _openDraftForPick(PickList pick) async {
    final cc = _companyCode;
    final label = pick.pickPreLabel;
    if (cc == null || label == null) return;
    final pickItems = await isar.pickListItems
        ...
        .findAll();
    final live = pickItems.where((e) => e.status != 'X').toList();
    if (live.isEmpty) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Pick $label has no items to pack')),
        );
      }
      return;
    }

    // NOTE: no pack number yet — it's minted on Save so abandoning the draft
    // doesn't burn a running number.
    final now = DateTime.now();
    final items = <PackListItem>[];
    for (final pi in live) {
      items.add(PackListItem()
        ..companyCode = cc
        ..skuNo = pi.skuNo
        ..uom = pi.uom
        ..parentPreLabel = label // parent = the pick
        ..sourceSq = pi.parentPreLabel // the pick line's own source SQ
        ..locationCode = pi.locationCode
        ..status = 'A'
        ..factor = pi.factor
        ..qty = pi.qty ?? 0 // target = pick line's planned qty
        ..packQty = 0
        ..addedBy = _userId
        ..addedDate = now
        ..lastModifiedBy = _userId
  ...
  Future<void> _openDraftForSq(Quote sq, {String? focusPick}) async {
    if (_companyCode == null) return;
    // Pack number is minted on Save, not here.
    final now = DateTime.now();
    final draft = PackList()
  ...
  Future<void> _uploadOne(PackList p) async {
    final label = p.packPreLabel;
  ...
  Future<void> _loadLocal() async {
    ...
    final all = await isar.packLists
        .filter()
        .companyCodeEqualTo(_companyCode)
        .findAll();

    // Dedupe by Pack_PreLabel. Legacy rows saved with an autoIncrement id
    ...
    final byLabel = <String, PackList>{};
    final stragglers = <int>[];
    for (final p in all) {
      final label = p.packPreLabel ?? '';
      final canonicalId =
          BaseTransactionSyncService.packListId(p.companyCode ?? 0, label);
    ...
    if (stragglers.isNotEmpty) {
      await isar.writeTxn(() async {
        await isar.packLists.deleteAll(stragglers);
      });
    }

    final open = <PackList>[];
    final completed = <PackList>[];
    ...
    int cmp(PackList a, PackList b) {
      final ad = a.addedDate ?? a.addedDate;
      final bd = b.addedDate ?? b.addedDate;
  ...
  Future<void> _refreshInBackground() async {
    ...
    try {
      ...
      final up = await PackService().syncUnsyncedPacks(companyCode: _companyCode);
      ...
      print('⚠️ PackingPage refresh failed: $e');
    } finally {
  ...
  Future<void> _createEmptyPick() async {
    if (_companyCode == null) return;
    // Draft header — NOT persisted yet, and NO pack number yet: it's minted on
    ...
    await Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => PackDetailPage(pick: draft, draft: true)),
    );
    await _loadLocal();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      drawer: const AppNavDrawer(current: AppSection.packing),
      appBar: AppBar(
                ...
                children: [
                  ...
                  Expanded(
                    child: TabBarView(
                      controller: _tabs,
                      children: [
                        _listOrEmpty(_filter(_open), 'No pending packs',
                            'Scan a document or tap + to start one.',
                            Icons.inventory_2),
                        _listOrEmpty(_filter(_completed), 'Nothing packed yet',
                            'Completed and cancelled packs land here.',
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Tooltip(
            ...
            child: InkWell(
              ...
              child: SizedBox(
                width: 38,
                height: 38,
                child: Icon(Icons.swap_vert, size: 19, color: AppDesign.modPacking),
              ),
            ),
          ),
          ),
          ...
          Material(
            color: _anyFilter ? AppDesign.modPacking : AppDesign.modPackingBg,
            borderRadius: BorderRadius.circular(AppDesign.radiusSm),
            child: InkWell(
              borderRadius: BorderRadius.circular(AppDesign.radiusSm),
              ...
              child: SizedBox(
                ...
                child: Stack(
                  ...
                  children: [
                    Icon(Icons.tune,
                        size: 19,
                        color: _anyFilter ? Colors.white : AppDesign.modPacking),
                    if (_anyFilter)
                      Positioned(
                        top: 5,
                        right: 5,
    ...
    return Container(
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          _segBtn('Packed', _filter(_completed).length, 1),
        ]),
      ),
    );
  }
  ...
  Widget _activeFilterSummary() {
    ...
    return Container(
      ...
      child: Row(children: [
        Icon(Icons.filter_alt, size: 13, color: AppDesign.modPacking),
        const SizedBox(width: 5),
        Expanded(
          child: Text(parts.join(' · '),
              style: TextStyle(
                  fontSize: 11, fontWeight: FontWeight.w700, color: AppDesign.modPacking),
              maxLines: 1,
  ...
  Widget _filterTile(
    ...
    return ListTile(
      ...
      onTap: onTap,
      leading: Icon(icon,
          size: 19, color: active ? AppDesign.modPacking : AppDesign.inkSubtle),
      title: Text(label,
          style: TextStyle(
              fontSize: 13,
              fontWeight: active ? FontWeight.w800 : FontWeight.w600,
              color: active ? AppDesign.modPacking : AppDesign.ink)),
      trailing: Icon(
          active ? Icons.check_box_rounded : Icons.check_box_outline_blank,
          size: 20,
          color: active ? AppDesign.modPacking : AppDesign.inkSubtle),
    );
  }

  Widget _listOrEmpty(List<PackList> rows, String title, String hint, IconData icon) {
    if (rows.isEmpty) {
  ...
  Future<Map<int, _DocStat>> _lineCounts(List<PackList> rows) async {
    ...
    for (final p in rows) {
      final items = await isar.packListItems
          ...
          .companyCodeEqualTo(_companyCode)
          .and()
          .packPreLabelEqualTo(p.packPreLabel)
          .and()
          .not()
          .statusEqualTo('X')
          .findAll();
  ...
  Widget _pickRow(PackList p, _DocStat st) {
    final sFg = _statusFg(p.status);
    final closed = p.status == 'C' || p.status == 'X';
    final who = p.assignedTo;
    final when = p.addedDate;
    final custs = _docCustomers[p.packPreLabel ?? ''] ?? const <String>[];
    ...
    return Material(
      ...
      child: InkWell(
        ...
        onTap: () async {
          await Navigator.push(
            context,
            MaterialPageRoute(builder: (_) => PackDetailPage(pick: p)),
          );
          await _loadLocal();
        },
        child: Container(
  ...
  Widget _syncControl(PackList p) {
  ...
  Color _statusFg(String? s) {
    switch (s) {
      case 'A': return AppDesign.modPacking;
